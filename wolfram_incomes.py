...
if __name__ == '__main__':
	...
	# with open ('data/counties.json') as file1:
	data1 = json.load(open('data/counties.json', 'r'), encoding='ISO-8859-1')
	print len(data1['features'])

	with open ('data/incomes_full.json') as file2:
	...
	for i in range(0, len(data1['features'])):
		...
		# raw_input()
	# print hospitals
	with open('counties4.json', 'w') as outfile:
	...
	# i=0
	# while i < len(data):
	# 	latlng_short.append(data[i])
	# 	i += 10
	# 	print i

	# with open('latlng_full_short.json', 'w') as outfile:
	# 	json.dump(latlng_short, outfile)
